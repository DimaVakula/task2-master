...
import {OverlayLoading} from "../../api/api";
...
const LocationResultList = (props) => {
    ...
    return (<SafeAreaView>
        <FlatList
            ...
            numColumns={2}
            renderItem={({item}) => <CityListItems title={item.name} icon={item.weather[0].icon}
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            {loading ? <OverlayLoading show={loading}/> :
                searchResult !== null ?
                    <LocationResultList loading={loading} onRefresh={onRefresh} data={searchResult}/>
